...
The built-in `Locale` type currently lists `en`, `tr`, and `zh-CN` for
...
1. Add a stable key to every registered locale package in
   `lib/i18n/messages/` (currently `en.ts`, `tr.ts`, and `zh-CN.ts`).
